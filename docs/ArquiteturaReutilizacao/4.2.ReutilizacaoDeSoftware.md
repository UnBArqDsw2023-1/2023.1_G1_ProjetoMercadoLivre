...
Dentro do escopo executado, pode-se perceber a utilização deste padrão de reutilização ao longo do fluxo de Login, onde, com base no protótipo de alta fidelidade, a biblioteca foi utilizada dentro do FrontEnd, permitindo a criação de componentes customizáveis - como por exemplo no Header e no Footer.
...
Ademais, o conteúdo criado dentro do arquivo Content, por se tratar de um fluxo de Login básico, pode ser reutilizado em outras aplicações que possuem a mesma necessidade fazendo as alterações desejadas - incluindo a substituição dos dados mocados por uma conexão com uma API advinda de requisições feitas através de um sistema Back End próprio conectado ao banco de dados de preferência.

#### Tipografia

Uma das estratégias utilizadas para reutilizar a tipografia do projeto, foi buscar a semelhante a  utilizada no Mercado Livre, e importá-la contendo as especificações correspondentes e aplicando-a como um padrão para a todas as páginas criadas.
